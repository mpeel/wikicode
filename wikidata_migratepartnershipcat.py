...
query = 'SELECT DISTINCT ?item WHERE {	?item wdt:P8252 ?value . }'
if debug:
	query = query + " LIMIT 1"
	...
	for wd_item in repeat_next:
		...
		for clm in P8252:
			...
			if existing_id == False:
				...
				items.append(['P971','Q97580368']) # combines partnership
			...
			newclaim = pywikibot.Claim(repo, 'P8464')
			newclaim.setTarget(wd_item2)
			print(newclaim)
			# test = input('Save new P7867 value?')
			# if test == 'y':
			wd_item.addClaim(newclaim, summary=u'Setting P8464 value')
			wd_item.removeClaims(clm, summary=u"Remove obsolete P8252 value")
